			...
			if (robustKernelDelta_ > 0.0) {
				for (auto iter = edges.begin(); iter != edges.end(); ++iter) {
					if ((*iter)->level() == 0 && (*iter)->chi2() > (*iter)->robustKernel()->delta()) {
						...
						// std::cout << "Ignoring edge " << (*iter)->vertex(0)->id()-stepVertexId << "<->" << (*iter)->vertex(1)->id() << " d: " << d << "  var: " << 1.0/((g2o::EdgeStereoSE3ProjectXYZ*)(*iter))->information()(0,0) << " kernel: " << (*iter)->robustKernel()->delta() << " chi2: " << (*iter)->chi2() << std::endl;
						Eigen::Vector3d pt3d;
						if ((*iter)->vertex(0)->id() > negVertexOffset) {
							pt3d = _points3D.at(negVertexOffset - (*iter)->vertex(0)->id());
						} else {
